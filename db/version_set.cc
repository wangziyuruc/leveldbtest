  ...
  void SaveTo(Version *v)
  {
    ...
    for (int level = 0; level < config::kNumLevels; level++)
    {
      ...
      for (int j = 0; j < levels_[level].added_buffer_nodes.size(); j++)
      {
        ...
        if (v->files_in_ssd_[level - 1].find(s) == v->files_in_ssd_[level - 1].end())
        { // this file never save
        ...
        std::cout << "mode : " << vset_->readModeWrite << std::endl;
        std::cout << "readNum : " << vset_->readNum << std::endl;
        std::cout << "writeNum : " << vset_->writeNum << std::endl;
        int kThresholdBufferNum = 0;
        switch (vset_->readModeWrite)
        {
        case 1:
          kThresholdBufferNum = config::kHighThresholdBufferNum;
          break;
        case 2:
          kThresholdBufferNum = config::kmiddleThresholdBufferNum;
          break;
        case 3:
        ...
        if (f->buffer->nodes.size() >= kThresholdBufferNum)
        {
          vset_->buffer_compact_switch_ = true;
          v->bc_compaction_level_ = level;
          if (std::find(v->need_compact_[level].begin(), v->need_compact_[level].end(), f) == v->need_compact_[level].end())
...
void VersionSet::Finalize(Version *v)
{
  ...
  for (int level = 0; level < config::kNumLevels - 1; level++)
  {
    ...
    else
    {
      ...
      {
        std::cout<<"level :"<<level<<" size: "<<static_cast<double>(level_bytes) <<std::endl;
        if (v->vset_->readModeWrite == 5)
        {
          if (static_cast<double>(level_bytes) != 0 && level == 1)
          {
            level = 1;
            score = 5;
          }
          else if (static_cast<double>(level_bytes) != 0 && level == 2)
          {
            level = 2;
            score = 4;
          } else {
        ...
        else if (v->vset_->readModeWrite == 6)
        {
          if (static_cast<double>(level_bytes) != 0 && level == 1)
          {
            level = 1;
          ...
          else if (static_cast<double>(level_bytes) != 0 && level == 3)
          {
            level = 3;
            score = 3;
          }
          else if (static_cast<double>(level_bytes) != 0 && level == 4)
          {
            level = 4;
            score = 2;
          } else {
...
bool Compaction::IsTrivialMove() const
{
  ...
  if (BCJudge::IsBufferCompactLevel(level_) &&
      level_ + 1 < config::kNumLevels && vset->current_->files_[level_ + 1].size() == 0)
    return true;

  return (num_input_files(0) == 1 && num_input_files(1) == 0 &&
          TotalFileSize(grandparents_) <=
              MaxGrandParentOverlapBytes(vset->options_) &&
          !(BCJudge::IsBufferCompactLevel(level_)));
}
