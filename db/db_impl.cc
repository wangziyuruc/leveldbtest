...
void DBImpl::BackgroundCompaction()
{
  ...
  else if (!is_manual && c->IsTrivialMove())
  {
    ...
    c->edit()->DeleteFile(c->level(), f->number); //delete file for level n
    c->edit()->AddFile(c->level() + 1, f->number, f->file_size,
                       f->smallest, f->largest); // add file level n+1
  ...
  else
  {
    ...
    { // wzy add
      ...
      {
        std::cout << " BackgroundCompaction read size: " << levelFile[i]->read_size << std::endl;
        if (levelFile[i]->read_size >= 10)
        {
