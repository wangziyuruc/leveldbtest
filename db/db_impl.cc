...
struct DBImpl::PartialCompactionStats
{
  ...
  PartialCompactionStats() : micros(0),
                             bytes_read(0),
                             bytes_written(0),
                             compact_times(0),
                             read_file_nums(0),
...
struct DBImpl::OneTimeCompactionStats
{
  ...
  int64_t ll_file_num;
  int64_t hl_file_num;
  OneTimeCompactionStats() : partial_stats(),
                             ll_file_num(-1),
                             hl_file_num(-1) {}
};
...
Status DBImpl::InstallCompactionResults(CompactionState *compact)
{
  ...
  if (!BCJudge::IsBufferCompactLevel(level))
    ...
    compact->compaction->AddInputUpDeletions(compact->compaction->edit());
  //const int level = compact->compaction->level();
  //whc change
  for (size_t i = 0; i < compact->outputs.size(); i++)
  {
...
// wzy
Status DBImpl::RemoveBuffer(CompactionState *compact)
{
  const uint64_t start_micros = env_->NowMicros();
  int64_t imm_micros = 0; // Micros spent doing imm_ compactions
  ...
  Compaction *c;
  c = versions_->PickCompaction();
  CompactionState *compact1 = new CompactionState(c);
  for (int index = 0; index < compact1->compaction->inputs_[0].size(); index++)
  {
    status = BufferCompact(compact1, index);
    if (!status.ok())
      break;
  }
  ...
  delete compact1->outfile;
  compact1->outfile = NULL;
  status = InstallCompactionResults(compact1);
  c->ReleaseInputs();
  delete c;
  ...
  VersionSet::LevelSummaryStorage tmp;
  Log(options_.info_log,
      "removebuffer to: %s", versions_->LevelSummary(&tmp));

  std::cout << "removebuffer end!" << std::endl;
  //for(int i=0;i<config::kNumLevels;i++)
  //std::cout<<"level"<<i<<"  nums"<<versions_->current_->NumFiles(i)<<std::endl;
  return status;
}
                   ...
                   std::string *value)
{
  ...
  versions_->readNum++;
  std::cout<<"Get readNum :"<<versions_->readNum<<std::endl;
  mem->Ref();
  if (imm != NULL)
    imm->Ref();
  current->Ref();
